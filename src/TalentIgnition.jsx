...
const IMG = {
  ...
  heroAlt: "https://images.unsplash.com/photo-1488190211105-8b0e65b80b4e?w=900&q=80",
  ...
  learning: "https://images.unsplash.com/photo-1588072432836-e10032774350?w=800&q=80",
  ...
  innovation: "https://images.unsplash.com/photo-1531746790731-6c087fecd65a?w=700&q=80",
  classroom: "https://images.unsplash.com/photo-1580582932707-520aed937b7b?w=1200&q=80",
  globe: "https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=700&q=80",
  india: "https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=900&q=80",
  mentor: "https://images.unsplash.com/photo-1560250097-0b93528c311a?w=700&q=80",
  test: "https://images.unsplash.com/photo-1606326608606-aa0b62935f2b?w=700&q=80",
  ...
  vision: "https://images.unsplash.com/photo-1509062522246-3755977927d7?w=900&q=80",
};
...
const Tag = ({ children, color = T.orange, textColor = "#fff", small = false }) => (
...
const Toast = ({ msg, onClose }) => (
  <div style={{ position: "fixed", bottom: 28, left: "50%", transform: "translateX(-50%)", background: T.ink, color: "#fff", padding: "14px 24px", borderRadius: 14, zIndex: 9999, fontSize: 13.5, fontWeight: 600, display: "flex", alignItems: "center", gap: 10, boxShadow: "0 12px 48px rgba(0,0,0,0.45)", whiteSpace: "nowrap", border: `1px solid ${T.borderDark}`, animation: "toastIn 0.35s cubic-bezier(0.34,1.56,0.64,1)" }}>
    ...
    {msg}
    <button style={{ background: "none", border: "none", color: "#666", cursor: "pointer", marginLeft: 4, fontSize: 20, lineHeight: 1 }} onClick={onClose}>×</button>
  </div>
);
...
const Modal = ({ title, subtitle, children, isOpen, onClose }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(8,12,26,0.82)", zIndex: 9000, display: "flex", alignItems: "flex-end", justifyContent: "center", backdropFilter: "blur(8px)" }} onClick={onClose}>
...
const SectionHeader = ({ tag, tagColor = T.orange, title, subtitle, center = false }) => (
  <div style={{ textAlign: center ? "center" : "left", marginBottom: 48 }}>
    {tag && <Tag color={tagColor} style={{ marginBottom: 14 }}>{tag}</Tag>}
...
const Nav = ({ onNav, onOpenSignin, onOpenLogin, currentPage, loggedIn, user, onLogout }) => {
  ...
  const NavLink = ({ id, label, active }) => (
    <button onClick={() => { onNav(id); setFrameworkOpen(false); setProgramsOpen(false); }}
      style={{ background: active ? `${T.orange}12` : "none", border: "none", color: active ? T.orange : "#BFC5D2", padding: "7px 14px", borderRadius: 8, fontSize: 13, fontWeight: active ? 700 : 500, cursor: "pointer", fontFamily: "'Outfit', sans-serif", transition: "all 0.15s", letterSpacing: 0.1 }}
      onMouseEnter={e => { if (!active) e.currentTarget.style.color = "#fff"; }}
      onMouseLeave={e => { if (!active) e.currentTarget.style.color = "#BFC5D2"; }}
    >{label}</button>
  );

  return (
    <>
      <nav style={{ background: scrolled ? "rgba(8,12,26,0.97)" : T.ink, height: 68, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", position: "sticky", top: 0, zIndex: 1000, borderBottom: `1px solid ${scrolled ? T.borderDark : "rgba(30,38,64,0.6)"}`, backdropFilter: "blur(12px)", transition: "all 0.3s" }}>
        
        {/* Logo */}
        <div style={{ display: "flex", alignItems: "center", gap: 11, cursor: "pointer", flexShrink: 0 }} onClick={() => { onNav("home"); setMenuOpen(false); }}>
          <div style={{ width: 38, height: 38, background: `linear-gradient(135deg, ${T.orange} 0%, ${T.gold} 100%)`, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 15, color: "#fff", fontFamily: "'Outfit', sans-serif", letterSpacing: -0.5, boxShadow: `0 4px 14px ${T.orange}40` }}>Ti</div>
          <div>
            <div style={{ fontWeight: 800, fontSize: 17, color: "#fff", letterSpacing: -0.5, fontFamily: "'Outfit', sans-serif", lineHeight: 1 }}><span style={{ color: T.orange }}>Talent</span> Ignition</div>
            {!mobile && <div style={{ fontSize: 8, color: "#4A5568", letterSpacing: 2.2, textTransform: "uppercase", marginTop: 1 }}>Ignite · Evolve · Excel</div>}
          </div>
        </div>
        ...
        {!mobile && (
          <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
            <NavLink id="home" label="Home" active={currentPage === "home"} />
            
            {/* Framework Dropdown */}
            <div style={{ position: "relative" }} onMouseEnter={() => setFrameworkOpen(true)} onMouseLeave={() => setFrameworkOpen(false)}>
              <button style={{ background: ["ramanujan","dhrona","chanakya","aryabhata"].includes(currentPage) ? `${T.orange}12` : "none", border: "none", color: ["ramanujan","dhrona","chanakya","aryabhata"].includes(currentPage) ? T.orange : "#BFC5D2", padding: "7px 14px", borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: "pointer", fontFamily: "'Outfit', sans-serif", display: "flex", alignItems: "center", gap: 4 }}>
                Framework <Icon name="chevron" size={14} />
              </button>
              {frameworkOpen && (
                <div style={{ position: "absolute", top: "100%", left: 0, background: T.ink2, border: `1px solid ${T.borderDark}`, borderRadius: 16, padding: "8px", minWidth: 200, zIndex: 100, boxShadow: "0 20px 60px rgba(0,0,0,0.4)", animation: "fadeIn 0.15s ease" }}>
                  {RDCA.map(r => (
                    <div key={r.id} style={{ padding: "10px 14px", borderRadius: 10, cursor: "pointer", display: "flex", alignItems: "center", gap: 10, transition: "all 0.15s" }}
                      onClick={() => { onNav(r.id); setFrameworkOpen(false); }}
                      onMouseEnter={e => e.currentTarget.style.background = T.ink3}
                      onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                      <span style={{ fontSize: 16 }}>{r.emoji}</span>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 700, color: r.color }}>{r.name}</div>
                        <div style={{ fontSize: 10.5, color: "#4A5568" }}>{r.tagline}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ position: "relative" }} onMouseEnter={() => setProgramsOpen(true)} onMouseLeave={() => setProgramsOpen(false)}>
              <button style={{ background: currentPage === "combo" ? `${T.orange}12` : "none", border: "none", color: currentPage === "combo" ? T.orange : "#BFC5D2", padding: "7px 14px", borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: "pointer", fontFamily: "'Outfit', sans-serif", display: "flex", alignItems: "center", gap: 4 }}>
                Programs <Icon name="chevron" size={14} />
              </button>
              {programsOpen && (
                <div style={{ position: "absolute", top: "100%", left: 0, background: T.ink2, border: `1px solid ${T.borderDark}`, borderRadius: 16, padding: "8px", minWidth: 180, zIndex: 100, boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
                  {[["Grade 1–5", "Primary Excellence"], ["Grade 6–8", "Middle School Mastery"], ["Grade 9–10", "Board Prep & Career Clarity"], ["Grade 11–12", "College & Career Launch"]].map(([g, sub]) => (
                    <div key={g} style={{ padding: "10px 14px", borderRadius: 10, cursor: "pointer", transition: "all 0.15s" }}
                      onClick={() => { onNav("combo"); setProgramsOpen(false); }}
                      onMouseEnter={e => e.currentTarget.style.background = T.ink3}
                      onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "#E2E8F0" }}>{g}</div>
            ...
            <NavLink id="success" label="Success Stories" active={currentPage === "success"} />
            <NavLink id="about" label="About Us" active={currentPage === "about"} />
            <NavLink id="contact" label="Contact" active={currentPage === "contact"} />
          </div>
        )}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {!mobile && !loggedIn && (
            <>
              <button onClick={onOpenLogin} style={{ background: "transparent", border: `1.5px solid #2A3250`, color: "#CBD5E1", padding: "8px 18px", borderRadius: 9, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "'Outfit', sans-serif", transition: "all 0.2s", letterSpacing: 0.1 }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = T.orange; e.currentTarget.style.color = T.orange; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = "#2A3250"; e.currentTarget.style.color = "#CBD5E1"; }}>Log In</button>
              <Btn size="sm" onClick={onOpenSignin} style={{ letterSpacing: 0.2 }}>Get Started →</Btn>
            </>
          )}
          {!mobile && loggedIn && (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, background: T.ink3, border: `1px solid ${T.borderDark}`, borderRadius: 12, padding: "5px 14px 5px 7px", cursor: "pointer" }} onClick={() => onNav("dashboard")}>
                ...
                <span style={{ fontSize: 13, color: "#E2E8F0", fontWeight: 600 }}>{user?.name?.split(" ")[0]}</span>
              </div>
              <button onClick={onLogout} style={{ background: "none", border: `1px solid ${T.borderDark}`, color: "#64748B", padding: "8px 12px", borderRadius: 9, cursor: "pointer", fontSize: 12, fontFamily: "'Outfit', sans-serif", display: "flex", alignItems: "center", gap: 5 }}>
                <Icon name="logout" size={14} /> Logout
              </button>
            </div>
          )}
          <button style={{ background: "none", border: "none", color: "#94A3B8", cursor: "pointer", padding: "6px", display: mobile ? "flex" : "flex", alignItems: "center", marginLeft: mobile ? 0 : 4 }} onClick={() => setMenuOpen(true)}>
      ...
      {/* Mobile/Full Drawer */}
      {menuOpen && <div style={{ position: "fixed", inset: 0, background: "rgba(8,12,26,0.72)", zIndex: 1998, backdropFilter: "blur(4px)" }} onClick={() => setMenuOpen(false)} />}
      <div style={{ position: "fixed", top: 0, right: 0, width: mobile ? "100%" : 320, height: "100vh", background: T.ink2, zIndex: 1999, transform: menuOpen ? "translateX(0)" : "translateX(100%)", transition: "transform 0.3s cubic-bezier(0.4,0,0.2,1)", overflowY: "auto", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "20px 24px", borderBottom: `1px solid ${T.borderDark}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 800, color: "#fff", fontSize: 17 }}><span style={{ color: T.orange }}>Talent</span> Ignition</div>
          <button style={{ background: T.ink3, border: "none", color: "#94A3B8", cursor: "pointer", width: 34, height: 34, borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setMenuOpen(false)}><Icon name="x" size={18} /></button>
        </div>

        {loggedIn && (
          <div style={{ padding: "16px 24px", background: `${T.orange}08`, borderBottom: `1px solid ${T.borderDark}`, display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11.5, color: "#64748B" }}>{user?.grade} · {user?.plan}</div>
            </div>
          </div>
        )}

        <div style={{ padding: "20px 16px", flex: 1 }}>
          <div style={{ fontSize: 9, letterSpacing: 2.2, textTransform: "uppercase", color: "#2A3A5C", padding: "0 8px 10px", fontWeight: 700 }}>Navigation</div>
          {[
            ...
            <div key={link.id} onClick={() => { onNav(link.id); setMenuOpen(false); }}
              ...
              onMouseEnter={e => { if (currentPage !== link.id) e.currentTarget.style.background = T.ink3; }}
              ...
              <span style={{ fontSize: 13.5, color: currentPage === link.id ? T.orange : "#94A3B8", fontWeight: currentPage === link.id ? 700 : 400 }}>{link.label}</span>
            </div>
          ))}
        </div>

        <div style={{ padding: 20, borderTop: `1px solid ${T.borderDark}` }}>
          {loggedIn ? (
            <Btn fullWidth variant="outline" style={{ borderColor: T.borderDark, color: "#94A3B8" }} onClick={() => { onLogout(); setMenuOpen(false); }}>
              <Icon name="logout" size={15} /> Log Out
            </Btn>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <Btn fullWidth onClick={() => { onOpenSignin(); setMenuOpen(false); }}>Get Started Free →</Btn>
              <Btn fullWidth variant="outline" style={{ borderColor: T.borderDark, color: "#94A3B8" }} onClick={() => { onOpenLogin(); setMenuOpen(false); }}>Log In</Btn>
            </div>
          )}
        </div>
      </div>
...
// ── FOOTER ────────────────────────────────────────────────────────────────────
const Footer = ({ onNav }) => (
  <footer style={{ background: T.ink, borderTop: `1px solid ${T.borderDark}` }}>
    <div style={{ maxWidth: 1080, margin: "0 auto", padding: "64px 40px 0" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 48, marginBottom: 56 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <div style={{ width: 40, height: 40, background: `linear-gradient(135deg, ${T.orange}, ${T.gold})`, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 15, color: "#fff", fontFamily: "'Outfit', sans-serif" }}>Ti</div>
            <div>
              <div style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: 18, color: "#fff" }}><span style={{ color: T.orange }}>Talent</span> Ignition</div>
              <div style={{ fontSize: 8, color: "#2A3A5C", letterSpacing: 2.5, textTransform: "uppercase" }}>Ignite · Evolve · Excel</div>
            </div>
          </div>
          <p style={{ fontSize: 13, color: "#4A5568", lineHeight: 1.85, marginBottom: 22 }}>India's most structured student growth platform for Grades 1–12. Turning raw potential into future leaders through the RDCA framework.</p>
          <div style={{ display: "flex", gap: 8 }}>
            {[["in","L"],["𝕏","X"],["▶","Y"],["ig","I"]].map(([s, key]) => (
              <div key={key} style={{ width: 36, height: 36, borderRadius: 9, background: T.ink3, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#4A5568", cursor: "pointer", transition: "all 0.2s", border: `1px solid ${T.borderDark}` }}
                ...
                onMouseLeave={e => { e.currentTarget.style.background = T.ink3; e.currentTarget.style.color = "#4A5568"; e.currentTarget.style.borderColor = T.borderDark; }}>
                {s}
              </div>
            ))}
          </div>
        ...
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2.2, textTransform: "uppercase", color: "#2A3A5C", marginBottom: 18 }}>Framework</div>
          {[["Ramanujan — Intelligence", "ramanujan"], ["Dhrona — Wisdom", "dhrona"], ["Chanakya — Leadership", "chanakya"], ["Aryabhata — Innovation", "aryabhata"]].map(([l, id]) => (
            <div key={id} onClick={() => onNav(id)} style={{ fontSize: 13, color: "#4A5568", marginBottom: 12, cursor: "pointer", transition: "color 0.2s" }}
              onMouseEnter={e => e.target.style.color = "#CBD5E1"}
              onMouseLeave={e => e.target.style.color = "#4A5568"}>{l}</div>
          ))}
        </div>

        <div>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2.2, textTransform: "uppercase", color: "#2A3A5C", marginBottom: 18 }}>Quick Links</div>
          {[["Home","home"],["Programs & Pricing","combo"],["Success Stories","success"],["About Us","about"],["Contact","contact"],["Privacy Policy","contact"],["Terms & Conditions","contact"]].map(([l, id]) => (
            <div key={l} onClick={() => onNav(id)} style={{ fontSize: 13, color: "#4A5568", marginBottom: 12, cursor: "pointer", transition: "color 0.2s" }}
              onMouseEnter={e => e.target.style.color = "#CBD5E1"}
              onMouseLeave={e => e.target.style.color = "#4A5568"}>{l}</div>
          ))}
        </div>
        ...
        <div>
          <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2.2, textTransform: "uppercase", color: "#2A3A5C", marginBottom: 18 }}>Contact Us</div>
          {[[<Icon name="mail" size={14} color="#4A5568" />, "hello@talentignition.in"],[<Icon name="phone" size={14} color="#4A5568" />, "+91 98765 43210"],[<Icon name="map" size={14} color="#4A5568" />, "Srikakulam, Andhra Pradesh"]].map(([icon, text], i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
              {icon}
              <span style={{ fontSize: 13, color: "#4A5568" }}>{text}</span>
            </div>
          ))}
          <div style={{ marginTop: 24, padding: "14px 16px", background: T.ink3, borderRadius: 14, border: `1px solid ${T.borderDark}` }}>
            <div style={{ fontSize: 10.5, color: "#64748B", marginBottom: 8, fontWeight: 700, letterSpacing: 0.5 }}>SCHOOL HOURS</div>
            <div style={{ fontSize: 12.5, color: "#94A3B8" }}>Mon–Sat · 9 AM – 7 PM IST</div>
          </div>
        </div>
      </div>
    </div>
    <div style={{ borderTop: `1px solid ${T.borderDark}`, padding: "20px 40px" }}>
      <div style={{ maxWidth: 1080, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        <div style={{ fontSize: 12, color: "#2A3A5C" }}>© 2025 Talent Ignition Pvt. Ltd. · All rights reserved.</div>
        <div style={{ fontSize: 12, color: "#2A3A5C" }}>🇮🇳 Empowering Future Leaders of India</div>
      </div>
    </div>
  </footer>
);
...
// ── RDCA CARD ─────────────────────────────────────────────────────────────────
const RDCACard = ({ r, onNav, large = false }) => {
  const [hov, setHov] = useState(false);
  return (
    <div style={{ background: r.bg, border: `1.5px solid ${hov ? r.color : r.border}`, borderRadius: 22, padding: large ? "28px 24px" : "24px 20px", cursor: "pointer", transition: "all 0.28s cubic-bezier(0.4,0,0.2,1)", transform: hov ? "translateY(-6px)" : "none", boxShadow: hov ? `0 24px 56px ${r.color}25` : "0 2px 8px rgba(0,0,0,0.04)", position: "relative", overflow: "hidden" }}
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} onClick={() => onNav(r.id)}>
      <div style={{ position: "absolute", right: -10, bottom: -10, fontFamily: "'Outfit', sans-serif", fontSize: large ? 100 : 80, fontWeight: 900, color: r.color, opacity: 0.05, lineHeight: 1, pointerEvents: "none", userSelect: "none" }}>{r.letter}</div>
      <div style={{ fontSize: large ? 32 : 28, marginBottom: 12 }}>{r.emoji}</div>
      <Tag color={r.color} small>{r.purpose}</Tag>
      <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: large ? 22 : 18, fontWeight: 800, color: T.ink, margin: "10px 0 4px", letterSpacing: -0.3 }}>{r.name}</h3>
      <div style={{ fontSize: 12, color: r.textColor, fontWeight: 700, letterSpacing: 0.3, marginBottom: 10 }}>{r.tagline}</div>
      <p style={{ fontSize: 13, color: T.muted, lineHeight: 1.65, marginBottom: 18 }}>{r.desc}</p>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 11, color: r.textColor, fontWeight: 600, background: `${r.color}12`, padding: "3px 10px", borderRadius: 20 }}>{r.grade}</span>
        <div style={{ width: 32, height: 32, borderRadius: "50%", background: `${r.color}15`, display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s", transform: hov ? "translateX(4px)" : "none" }}>
          <Icon name="arrow" size={15} color={r.color} />
        </div>
      </div>
    </div>
  );
...
const HomePage = ({ onNav, onOpenLogin, onOpenSignin, loggedIn, user }) => {
  ...
  const [counter, setCounter] = useState({ students: 0, projects: 0, assessments: 0, rate: 0 });
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting && !counted.current) {
        ...
        const targets = { students: 12000, projects: 3400, assessments: 28000, rate: 97 };
        let start = null;
        const duration = 1800;
        const animate = (ts) => {
          if (!start) start = ts;
          ...
          setCounter({ students: Math.floor(ease * targets.students), projects: Math.floor(ease * targets.projects), assessments: Math.floor(ease * targets.assessments), rate: Math.floor(ease * targets.rate) });
  ...
  return (
    <div>
      {/* ── HERO ── */}
      <section style={{ background: T.ink, minHeight: mobile ? "auto" : "95vh", display: "flex", alignItems: "center", position: "relative", overflow: "hidden", padding: mobile ? "72px 20px 60px" : "0 40px" }}>
        <div style={{ position: "absolute", inset: 0, backgroundImage: `url(${IMG.hero})`, backgroundSize: "cover", backgroundPosition: "center 20%", opacity: 0.08 }} />
        <div style={{ position: "absolute", inset: 0, background: `linear-gradient(135deg, ${T.ink} 0%, rgba(8,12,26,0.94) 55%, rgba(8,12,26,0.75) 100%)` }} />
        <div style={{ position: "absolute", top: -200, right: -100, width: 700, height: 700, background: `radial-gradient(circle, ${T.orange}20 0%, transparent 60%)`, pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: -100, left: -100, width: 500, height: 500, background: `radial-gradient(circle, ${T.teal}12 0%, transparent 60%)`, pointerEvents: "none" }} />
        <div style={{ position: "absolute", top: "40%", right: "40%", width: 250, height: 250, background: `radial-gradient(circle, ${T.blue}08 0%, transparent 70%)`, pointerEvents: "none" }} />
        ...
        <div style={{ maxWidth: 1080, margin: "0 auto", width: "100%", display: mobile ? "block" : "flex", alignItems: "center", gap: 60, position: "relative", zIndex: 1 }}>
          <div style={{ flex: 1, maxWidth: 620 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: `${T.orange}14`, border: `1px solid ${T.orange}28`, borderRadius: 30, padding: "6px 16px 6px 8px", marginBottom: 28 }}>
              <span style={{ width: 8, height: 8, borderRadius: "50%", background: T.orange, display: "inline-block", boxShadow: `0 0 8px ${T.orange}` }} />
              <span style={{ fontSize: 12, color: T.orange, fontWeight: 700, letterSpacing: 0.3 }}>India's #1 Student Growth Platform</span>
            </div>
            
            <h1 style={{ fontFamily: "'Outfit', sans-serif", fontSize: mobile ? "clamp(36px,10vw,42px)" : "clamp(46px,5vw,68px)", fontWeight: 900, color: "#fff", letterSpacing: -2.5, lineHeight: 1.0, marginBottom: 24, textShadow: "0 2px 40px rgba(0,0,0,0.3)" }}>
              Ignite Your<br />
              <span style={{ background: `linear-gradient(90deg, ${T.orange}, ${T.gold})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Child's Potential.</span>
              <br />Transform Their Future.
            </h1>

            <p style={{ fontSize: mobile ? 16 : 18, color: "#94A3B8", lineHeight: 1.75, marginBottom: 38, maxWidth: 520 }}>
              The RDCA Framework — Ramanujan, Dhrona, Chanakya & Aryabhata — gives every student a complete growth journey from Grade 1 to Grade 12.
            </p>

            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 48 }}>
              <Btn size="xl" onClick={onOpenSignin} style={{ letterSpacing: 0.2 }}>
                🎯 Start Free Today
              </Btn>
              <Btn size="xl" variant="outlineWhite" onClick={() => onNav("about")}>
                <Icon name="play" size={14} color="#fff" /> Watch Story
              </Btn>
            </div>

            <div style={{ display: "flex", gap: mobile ? 28 : 44, paddingTop: 32, borderTop: `1px solid rgba(255,255,255,0.07)`, flexWrap: "wrap" }}>
              {[["12K+", "Students Enrolled"], ["4", "Growth Tracks"], ["Grade 1–12", "All Grades Covered"]].map(([n, l]) => (
                <div key={l}>
                  <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: mobile ? 24 : 30, fontWeight: 800, color: "#fff", letterSpacing: -1 }}>{n}</div>
                  <div style={{ fontSize: 12, color: "#4A5568", marginTop: 3, letterSpacing: 0.3 }}>{l}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {!mobile && (
            <div style={{ flex: "0 0 400px", position: "relative" }}>
              <div style={{ borderRadius: 28, overflow: "hidden", border: `2px solid rgba(255,255,255,0.06)`, boxShadow: "0 40px 100px rgba(0,0,0,0.65)", position: "relative" }}>
                <img src={IMG.student1} alt="Students learning together" style={{ width: "100%", height: 420, objectFit: "cover", display: "block" }} />
                <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(8,12,26,0.5) 0%, transparent 50%)" }} />
              </div>
              {/* Floating badges */}
              <div style={{ position: "absolute", top: -20, right: -20, background: `linear-gradient(135deg, ${T.orange}, #C94010)`, borderRadius: 18, padding: "14px 20px", boxShadow: `0 12px 36px ${T.orange}50`, zIndex: 3 }}>
                <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 26, fontWeight: 800, color: "#fff", lineHeight: 1 }}>4</div>
                <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.82)", lineHeight: 1.4, marginTop: 2 }}>RDCA<br/>Tracks</div>
              </div>
              <div style={{ position: "absolute", bottom: -20, left: -20, background: T.white, borderRadius: 18, padding: "14px 20px", boxShadow: "0 16px 40px rgba(0,0,0,0.22)", zIndex: 3, display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontSize: 26 }}>⭐</span>
                <div>
                  <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 20, fontWeight: 800, color: T.ink, lineHeight: 1 }}>4.9<span style={{ fontSize: 13, fontWeight: 500, color: T.muted }}>/5</span></div>
                  <div style={{ fontSize: 10.5, color: T.muted, marginTop: 1 }}>Parent Rating</div>
                </div>
              </div>
              <div style={{ position: "absolute", top: "47%", left: -26, background: T.ink2, border: `1px solid ${T.borderDark}`, borderRadius: 14, padding: "10px 16px", zIndex: 3, boxShadow: "0 8px 28px rgba(0,0,0,0.4)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ width: 9, height: 9, borderRadius: "50%", background: T.green2, boxShadow: `0 0 10px ${T.green2}`, flexShrink: 0 }} />
                  <span style={{ fontSize: 12, fontWeight: 600, color: "#CBD5E1" }}>12,480 Active Students</span>
                </div>
              </div>
            </div>
          )}
      ...
      {/* ── TRUST BAR ── */}
      <div style={{ background: T.ink2, borderBottom: `1px solid ${T.borderDark}`, padding: "14px 24px" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", display: "flex", alignItems: "center", gap: 32, overflowX: "auto", scrollbarWidth: "none" }}>
          <div style={{ fontSize: 10, color: "#2A3A5C", fontWeight: 700, letterSpacing: 1.8, textTransform: "uppercase", flexShrink: 0 }}>Trusted by schools in</div>
          {["Delhi", "Mumbai", "Bangalore", "Hyderabad", "Chennai", "Kolkata", "Pune", "Srikakulam"].map(city => (
            <div key={city} style={{ fontSize: 12.5, color: "#4A5568", fontWeight: 600, flexShrink: 0, whiteSpace: "nowrap" }}>📍 {city}</div>
          ))}
        </div>
      </div>

      {/* ── GRADE STRIP ── */}
      <div style={{ background: T.cream2, padding: "14px 20px", borderBottom: `1px solid ${T.border}`, overflowX: "auto", scrollbarWidth: "none" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", display: "flex", gap: 6, flexWrap: "nowrap" }}>
          <div style={{ fontSize: 10.5, color: T.muted, fontWeight: 700, display: "flex", alignItems: "center", paddingRight: 8, borderRight: `1px solid ${T.border}`, marginRight: 4, flexShrink: 0 }}>EXPLORE BY GRADE</div>
          {GRADES.map(g => (
            <button key={g.grade} onClick={() => setActiveGrade(activeGrade === g.grade ? null : g.grade)}
              style={{ padding: "6px 14px", borderRadius: 22, fontSize: 12, fontWeight: 600, background: activeGrade === g.grade ? T.orange : T.white, color: activeGrade === g.grade ? "#fff" : T.ink, cursor: "pointer", border: `1.5px solid ${activeGrade === g.grade ? T.orange : T.border}`, transition: "all 0.2s", fontFamily: "'Outfit', sans-serif", flexShrink: 0, whiteSpace: "nowrap" }}>
              {g.grade}
            </button>
          ))}
        </div>
      ...
      {activeGrade && (() => {
        ...
        return (
          <div style={{ background: T.white, borderBottom: `2px solid ${T.orange}`, padding: "18px 24px", borderTop: "none" }}>
            <div style={{ maxWidth: 1080, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 14 }}>
              <div>
                <span style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: 16, color: T.ink }}>{activeGrade}</span>
                <span style={{ fontSize: 13, color: T.muted, marginLeft: 10 }}>· {g.level} · {g.range}</span>
                <p style={{ fontSize: 13.5, color: T.muted, marginTop: 5 }}>Full RDCA curriculum access — assessments, mentorship, career guidance, and AI labs tailored for this grade level.</p>
              </div>
              <Btn onClick={onOpenSignin}>Start {activeGrade} Journey →</Btn>
            </div>
          </div>
        );
      })()}
      ...
      {/* ── RDCA SECTION ── */}
      <Section bg={T.cream} py="96px 40px">
        <SectionHeader tag="The RDCA Framework" tagColor={T.blue} title="Four Paths. One Complete Journey." subtitle="Named after India's greatest minds — each track unlocks a dimension of your child's potential." />
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: 20, marginBottom: 44 }}>
          {RDCA.map(r => <RDCACard key={r.id} r={r} onNav={onNav} />)}
        </div>
        <div style={{ background: T.ink, borderRadius: 24, padding: mobile ? "28px 20px" : "32px 40px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 20 }}>
          <div>
            <div style={{ fontSize: 12, color: "#4A5568", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 6 }}>The RDCA Journey</div>
            <div style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: 22, color: "#fff" }}>Assess → Aware → Align → Accelerate</div>
          </div>
          <div style={{ display: "flex", gap: 0, overflowX: "auto" }}>
            {RDCA.map((r, i) => (
              <div key={r.id} style={{ display: "flex", alignItems: "center" }}>
                <div style={{ background: r.color, color: "#fff", borderRadius: 10, padding: "10px 18px", textAlign: "center", cursor: "pointer", fontFamily: "'Outfit', sans-serif", fontSize: 14, fontWeight: 800, whiteSpace: "nowrap", transition: "all 0.2s" }}
                  onClick={() => onNav(r.id)}
                  onMouseEnter={e => e.currentTarget.style.transform = "scale(1.05)"}
                  onMouseLeave={e => e.currentTarget.style.transform = "none"}>
                  {r.letter}
                </div>
                {i < 3 && <div style={{ color: "#2A3A5C", margin: "0 6px", fontSize: 18 }}>→</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── WHY TI ── */}
      <Section bg={T.white} py="96px 40px">
        <SectionHeader tag="Why Talent Ignition" title="Beyond Marks. Building Whole Students." subtitle="Most EdTech platforms only focus on exam scores. We focus on the whole student — intelligence, wisdom, leadership, and innovation." />
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(3, 1fr)", gap: 22 }}>
          {[
            { icon: "brain", title: "Not Just Marks", desc: "We develop cognitive ability, self-awareness, and life skills alongside academic excellence.", color: T.orange },
            { icon: "map", title: "A Personalised Roadmap", desc: "Every student gets a custom journey — from where they are to where they want to be.", color: T.blue },
            { icon: "users", title: "Real Human Mentors", desc: "IITians, doctors, artists and entrepreneurs mentor your child — not just pre-recorded videos.", color: T.green },
            { icon: "target", title: "Career-Ready from Grade 6", desc: "Career clarity isn't for college. We start mapping ambitions from middle school onwards.", color: T.purple },
            { icon: "zap", title: "Future Skills Built-In", desc: "AI literacy, design thinking, coding and innovation are woven into every learning path.", color: T.gold },
            { icon: "award", title: "Certified Progress", desc: "Every milestone is certified and recognized — building a portfolio from school itself.", color: T.teal },
          ].map(item => (
            <div key={item.title} style={{ padding: "26px", border: `1.5px solid ${T.border}`, borderRadius: 20, display: "flex", gap: 18, alignItems: "flex-start", transition: "all 0.25s", background: T.white }}
              onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 12px 40px rgba(0,0,0,0.07)"; e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.borderColor = item.color; }}
              ...
              <div style={{ width: 48, height: 48, borderRadius: 14, background: `${item.color}12`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: `1px solid ${item.color}18` }}>
                <Icon name={item.icon} size={22} color={item.color} />
              </div>
              <div>
                <div style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 700, fontSize: 15.5, marginBottom: 6, color: T.ink }}>{item.title}</div>
                <div style={{ fontSize: 13.5, color: T.muted, lineHeight: 1.7 }}>{item.desc}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ── STATS ── */}
      <section ref={statsRef} style={{ background: T.ink, padding: mobile ? "56px 20px" : "96px 40px", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, backgroundImage: `url(${IMG.classroom})`, backgroundSize: "cover", backgroundPosition: "center", opacity: 0.07 }} />
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(8,12,26,0.97) 0%, rgba(8,12,26,0.88) 100%)" }} />
        <div style={{ maxWidth: 1080, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <SectionHeader tag="Our Impact" title="Numbers That Speak for Themselves" subtitle="Real students, real results, real transformation." />
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: 20, marginBottom: 56 }}>
            {[
              { value: counter.students.toLocaleString() + "+", label: "Students Enrolled", sub: "Across India", icon: "users", color: T.orange },
              { value: counter.projects.toLocaleString() + "+", label: "Projects Completed", sub: "Innovation & AI Labs", icon: "rocket", color: T.gold },
              { value: counter.assessments.toLocaleString() + "+", label: "Assessments Done", sub: "PET & Pr.ET Tests", icon: "chart", color: T.teal },
              { value: counter.rate + "%", label: "Success Rate", sub: "Parent Satisfaction", icon: "star", color: T.purple },
            ].map(s => (
              <div key={s.label} style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${T.borderDark}`, borderRadius: 22, padding: "28px 22px", textAlign: "center", transition: "all 0.25s" }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = `${s.color}40`; e.currentTarget.style.background = `${s.color}06`; e.currentTarget.style.transform = "translateY(-3px)"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = T.borderDark; e.currentTarget.style.background = "rgba(255,255,255,0.03)"; e.currentTarget.style.transform = "none"; }}>
                <div style={{ width: 52, height: 52, borderRadius: 16, background: `${s.color}16`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                  <Icon name={s.icon} size={24} color={s.color} />
                </div>
                <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 36, fontWeight: 900, color: s.color, letterSpacing: -1.5, lineHeight: 1 }}>{s.value}</div>
                <div style={{ fontSize: 14, color: "#CBD5E1", fontWeight: 600, marginTop: 8 }}>{s.label}</div>
                <div style={{ fontSize: 11.5, color: "#4A5568", marginTop: 4 }}>{s.sub}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 20 }}>
            {[
              { emoji: "🎯", title: "Our Mission", color: T.orange, text: "To give every Indian student — regardless of background — the tools, guidance, and inspiration to discover their unique potential and build a future they are proud of." },
              { emoji: "🌟", title: "Our Vision", color: T.blue, text: "A generation of young Indians who graduate not just exam-ready, but world-ready — equipped with intelligence, wisdom, leadership, and innovation to lead tomorrow." },
            ].map(item => (
              <div key={item.title} style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${T.borderDark}`, borderRadius: 22, padding: "28px 26px" }}>
                <div style={{ fontSize: 32, marginBottom: 14 }}>{item.emoji}</div>
                <div style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: 20, color: item.color, marginBottom: 12 }}>{item.title}</div>
                <p style={{ fontSize: 14.5, color: "#94A3B8", lineHeight: 1.8 }}>{item.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── TESTIMONIALS ── */}
      <Section bg={T.white} py="96px 40px">
        <SectionHeader tag="Success Stories" tagColor={T.green} title="What Families Are Saying" subtitle="Real stories from parents and students across India who transformed their journey with RDCA." center />
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(3, 1fr)", gap: 22, marginBottom: 40 }}>
          {[
            { name: "Priya Sharma", role: "Parent · Grade 9", text: "My daughter's self-confidence has completely transformed. The Chanakya career mapping gave her clarity we couldn't provide. She now knows exactly where she's headed and is working towards it every day.", img: IMG.parent1, stars: 5, city: "Delhi", stat: "Grade improved from 68% to 89%" },
            { name: "Arun Mehta", role: "Parent · Grade 11", text: "The PET assessment revealed our son's hidden strengths. The Ramanujan mock tests are brilliantly designed. The progress reports are so detailed and actionable. Absolutely worth every rupee invested.", img: IMG.parent2, stars: 5, city: "Mumbai", stat: "Cleared JEE Mains on first attempt" },
            { name: "Sujatha Iyer", role: "Parent · Grade 7", text: "The Dhrona mentorship sessions are incredible — my child comes home inspired every time. The IAU sessions expanded his worldview in ways school never did. The AI learning track is truly ahead of its time.", img: IMG.parent3, stars: 5, city: "Bangalore", stat: "Won school Science Olympiad" },
            { name: "Kavitha Nair", role: "Parent · Grade 5", text: "Starting early with Talent Ignition was the best decision. My daughter has developed critical thinking and curiosity that her school friends simply don't have. The assessments helped us understand her learning style.", img: IMG.parent4, stars: 5, city: "Kochi", stat: "Became school debate champion" },
            { name: "Ravi Teja", role: "Parent · Grade 10", text: "The Aryabhata AI labs got my son so excited about technology. He built his first mobile app at age 15! The mentors are real professionals who genuinely invest in these kids.", img: IMG.parent5, stars: 5, city: "Hyderabad", stat: "Built and launched 2 mobile apps" },
            { name: "Deepak Verma", role: "Parent · Grade 12", text: "The Chanakya interview prep sessions are exceptional. My son cleared 3 competitive exams in a row. The resume building workshop helped him secure an early internship offer from a tech startup.", img: IMG.parent6, stars: 5, city: "Pune", stat: "Secured internship at a startup" },
          ].map(t => (
            <div key={t.name} style={{ border: `1.5px solid ${T.border}`, borderRadius: 22, padding: "26px", background: T.cream, transition: "all 0.25s" }}
              onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 16px 48px rgba(0,0,0,0.09)"; e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.borderColor = T.orange + "44"; }}
              onMouseLeave={e => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.transform = "none"; e.currentTarget.style.borderColor = T.border; }}>
              <div style={{ display: "flex", gap: 2, marginBottom: 14 }}>
                {Array.from({ length: t.stars }).map((_, i) => <Icon key={i} name="star" size={15} color={T.gold} />)}
              </div>
              <p style={{ fontSize: 14, color: T.ink, lineHeight: 1.78, marginBottom: 18, fontStyle: "italic", opacity: 0.88 }}>"{t.text}"</p>
              <div style={{ background: `${T.green}10`, border: `1px solid ${T.green}25`, borderRadius: 10, padding: "7px 12px", fontSize: 11.5, color: T.green, fontWeight: 700, marginBottom: 16 }}>
                ✓ {t.stat}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 12, paddingTop: 14, borderTop: `1px solid ${T.border}` }}>
                ...
                <div>
                  <div style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 700, fontSize: 14, color: T.ink }}>{t.name}</div>
                  <div style={{ fontSize: 11.5, color: T.muted }}>{t.role} · {t.city}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ textAlign: "center" }}>
          <Btn variant="outline" size="lg" onClick={() => onNav("success")}>View All Success Stories →</Btn>
        </div>
      </Section>

      {/* ── STUDENT TRANSFORMATION JOURNEY ── */}
      <Section bg={T.cream} py="96px 40px">
        <SectionHeader tag="Student Journey" tagColor={T.purple} title="From Uncertainty to Clarity" subtitle="See how our RDCA framework transforms students across every grade level." center />
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(4, 1fr)", gap: 0, position: "relative" }}>
          <div style={{ position: "absolute", top: "28px", left: "12.5%", right: "12.5%", height: 2, background: `linear-gradient(90deg, ${T.orange}, ${T.blue}, ${T.green}, ${T.purple})`, opacity: 0.3, display: mobile ? "none" : "block" }} />
          {[
            { step: "01", title: "Assess", desc: "Take the PET — discover your true academic profile, strengths, and learning style.", color: T.orange, emoji: "🧮" },
            { step: "02", title: "Explore", desc: "Monthly IAU sessions expand your worldview and expose you to global thinking.", color: T.blue, emoji: "📚" },
            { step: "03", title: "Align", desc: "Career mapping creates a personalised roadmap aligned to your passions.", color: T.green, emoji: "🎯" },
            { step: "04", title: "Accelerate", desc: "AI labs, hackathons and coding projects build your future-ready portfolio.", color: T.purple, emoji: "🚀" },
          ].map(s => (
            <div key={s.step} style={{ textAlign: "center", padding: "0 24px", position: "relative" }}>
              <div style={{ width: 56, height: 56, borderRadius: "50%", background: `${s.color}14`, border: `3px solid ${s.color}40`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 20px", fontSize: 24, position: "relative", zIndex: 1, background: T.white, boxShadow: `0 4px 20px ${s.color}25` }}>{s.emoji}</div>
              <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 11, fontWeight: 800, color: s.color, letterSpacing: 2, marginBottom: 6 }}>STEP {s.step}</div>
              <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 18, fontWeight: 800, color: T.ink, marginBottom: 10 }}>{s.title}</div>
              <div style={{ fontSize: 13.5, color: T.muted, lineHeight: 1.7 }}>{s.desc}</div>
            </div>
          ))}
        </div>
      </Section>
      ...
      {/* ── CTA BANNER ── */}
      <section style={{ background: `linear-gradient(135deg, #C94010 0%, ${T.orange} 50%, #E85A1F 100%)`, padding: mobile ? "60px 20px" : "96px 40px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: -150, right: -150, width: 500, height: 500, background: "radial-gradient(circle, rgba(255,255,255,0.1) 0%, transparent 60%)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: -100, left: -100, width: 400, height: 400, background: "radial-gradient(circle, rgba(0,0,0,0.15) 0%, transparent 60%)", pointerEvents: "none" }} />
        <div style={{ maxWidth: 680, margin: "0 auto", position: "relative", zIndex: 1 }}>
          <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: 2.5, textTransform: "uppercase", color: "rgba(255,255,255,0.65)", marginBottom: 16 }}>Start Today — No Hidden Fees</div>
          <h2 style={{ fontFamily: "'Outfit', sans-serif", fontSize: mobile ? 30 : 48, fontWeight: 900, color: "#fff", marginBottom: 18, letterSpacing: -2, lineHeight: 1.1 }}>
            Ready to Ignite Your<br />Child's Potential?
          </h2>
          <p style={{ fontSize: 17, color: "rgba(255,255,255,0.8)", marginBottom: 40, lineHeight: 1.7 }}>Join 12,000+ students across India who are already on their RDCA journey.</p>
          <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
            <Btn size="xl" variant="white" onClick={onOpenSignin}>Create Free Account</Btn>
            <Btn size="xl" variant="outlineWhite" onClick={() => onNav("combo")}>View Plans</Btn>
          </div>
          <div style={{ marginTop: 32, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
            <Icon name="shield" size={14} color="rgba(255,255,255,0.6)" />
            <span style={{ fontSize: 12, color: "rgba(255,255,255,0.6)" }}>No credit card required · Cancel anytime</span>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1080, margin: "0 auto", position: "relative", zIndex: 1, display: mobile ? "block" : "flex", gap: 60, alignItems: "center" }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
              <div style={{ width: 52, height: 52, borderRadius: 16, background: r.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26 }}>{r.emoji}</div>
              <Tag color={r.color}>{r.purpose} · {r.grade}</Tag>
            </div>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              ...
              <Btn size="lg" variant="outline" onClick={() => onNav("home")}>← Back to Home</Btn>
            </div>
          </div>
          {!mobile && (
            <div style={{ flex: "0 0 380px" }}>
              <img src={r.id === "ramanujan" ? IMG.ramanujan : r.id === "dhrona" ? IMG.dhrona : r.id === "chanakya" ? IMG.chanakya : IMG.aryabhata} alt={r.name} style={{ width: "100%", height: 320, objectFit: "cover", borderRadius: 22, boxShadow: `0 24px 64px ${r.color}25`, border: `2px solid ${r.color}20` }} />
            </div>
          )}
        </div>
      </section>
                  ...
                  {[{ sub: "Mathematics", pet: 62, pret: 84, c: T.green }, { sub: "Science", pet: 70, pret: 88, c: T.green }, { sub: "English", pet: 75, pret: 85, c: T.teal }, { sub: "Logical Reasoning", pet: 58, pret: 79, c: T.green }, { sub: "Overall", pet: 66, pret: 84, c: T.orange }].map((row, i) => (
                    <tr key={row.sub} style={{ background: i % 2 === 0 ? "#fff" : T.cream }}>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <span style={{ background: `${row.c}15`, color: row.c, padding: "3px 10px", borderRadius: 10, fontSize: 12, fontWeight: 700 }}>+{row.pret - row.pet}%</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <Section bg={T.white} py="80px 40px">
        ...
        {selMonth && SESSION_DATA[selMonth] && (
          <div style={{ background: r.bg, border: `2px solid ${r.border}`, borderRadius: 22, padding: "28px 28px", marginBottom: 24, animation: "fadeIn 0.25s ease" }}>
            ...
            <div style={{ fontSize: 14, background: `${r.color}10`, borderRadius: 12, padding: "12px 16px", marginBottom: 20, color: T.ink, lineHeight: 1.6 }}>
              <strong>Featured Topic:</strong> {SESSION_DATA[selMonth].topic}
            </div>
  ...
  return (
    <div>
      <Section bg={T.ink} py="80px 40px" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(8,12,26,0.97), rgba(8,12,26,0.85))" }} />
      ...
      <Section bg={T.cream} py="80px 40px">
        ...
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(2, 1fr)", gap: 24 }}>
          {[
            { name: "Aarav Patel", grade: "Grade 12 · Delhi", img: IMG.parent1, achievement: "IIT JEE Mains Cleared", desc: "Started Ramanujan track in Grade 10. PET revealed his weak spots in Maths early enough to fix them. Cleared JEE Mains in first attempt. Currently preparing for Advanced.", track: "Ramanujan", color: T.orange },
            { name: "Sneha Krishnamurthy", grade: "Grade 11 · Bangalore", img: IMG.parent3, achievement: "Won National Science Olympiad", desc: "Aryabhata AI labs sparked her love for data science. She used skills from the AI sessions to build a climate prediction model that won at the national level.", track: "Aryabhata", color: T.purple },
            ...
            { name: "Ananya Iyer", grade: "Grade 12 · Hyderabad", img: IMG.parent5, achievement: "Secured Scholarship to NUS Singapore", desc: "Chanakya's career mapping and interview prep transformed her application. The LinkedIn personal brand workshop helped her get noticed by the NUS scholarship committee.", track: "Chanakya", color: T.green },
          ].map(s => (
      ...
      <Section bg={T.white} py="80px 40px">
        <SectionHeader tag="Parent Testimonials" tagColor={T.blue} title="What Parents Say" center />
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(3, 1fr)", gap: 22 }}>
          {[
            { name: "Priya Sharma", role: "Parent of Riya, Grade 9 · Delhi", img: IMG.parent1, text: "The transformation in my daughter is remarkable. She went from being uncertain about her future to having a clear 5-year plan. The Chanakya track is worth every rupee.", stars: 5 },
            { name: "Arun Mehta", role: "Parent of Karan, Grade 11 · Mumbai", img: IMG.parent2, text: "The PET report was eye-opening. It showed us exactly where our son stood and what he needed to work on. The mock tests are brilliantly calibrated to board exam standards.", stars: 5 },
            { name: "Sujatha Iyer", role: "Parent of Dev, Grade 7 · Bangalore", img: IMG.parent3, text: "Starting early was the best decision. The IAU sessions have made my son so curious about the world. He reads news and asks intelligent questions now. Completely transformed.", stars: 5 },
          ].map(t => (
            <div key={t.name} style={{ background: T.cream, border: `1.5px solid ${T.border}`, borderRadius: 22, padding: "26px" }}>
              <div style={{ display: "flex", gap: 2, marginBottom: 14 }}>
                {Array.from({ length: t.stars }).map((_, i) => <Icon key={i} name="star" size={16} color={T.gold} />)}
              </div>
              <p style={{ fontSize: 14, color: T.ink, lineHeight: 1.78, marginBottom: 20, fontStyle: "italic" }}>"{t.text}"</p>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <img src={t.img} alt={t.name} style={{ width: 46, height: 46, borderRadius: "50%", objectFit: "cover" }} />
                <div>
                  <div style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 700, fontSize: 14 }}>{t.name}</div>
                  <div style={{ fontSize: 11.5, color: T.muted }}>{t.role}</div>
                </div>
              </div>
            </div>
          ))}
  ...
  return (
    <div>
      <Section bg={T.ink} py="80px 40px" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(8,12,26,0.96), rgba(8,12,26,0.88))" }} />
        <div style={{ position: "relative", zIndex: 1, textAlign: "center" }}>
      ...
      <Section bg={T.cream} py="80px 40px">
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(3, 1fr)", gap: 24, marginBottom: 56 }}>
          {plans.map(p => (
            <div key={p.id} style={{ background: p.dark ? T.ink : T.white, border: `2.5px solid ${selected === p.id ? p.color : p.dark ? T.borderDark : T.border}`, borderRadius: 26, padding: "30px 26px", position: "relative", cursor: "pointer", transition: "all 0.28s", boxShadow: selected === p.id ? `0 20px 60px ${p.color}22` : "none", transform: selected === p.id && !mobile ? "scale(1.02)" : "none" }}
              ...
              <div style={{ marginBottom: 26 }}>
                {p.features.map(f => (
                  <div key={f} style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 10 }}>
                    <div style={{ width: 18, height: 18, borderRadius: "50%", background: `${p.color}18`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
              ...
              <Btn fullWidth size="lg" onClick={onOpenLogin} style={{ background: p.color, boxShadow: `0 6px 20px ${p.color}30` }}>
                Choose {p.name} →
              </Btn>
            </div>
          ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: 18 }}>
          {[
            { grade: "Grade 1–5", sub: "Primary Excellence", emoji: "🌱", color: T.teal, desc: "Foundation skills, reading, world awareness, and curiosity-building through play-based learning." },
  ...
  return (
    <div>
      <Section bg={T.ink} py="80px 40px" style={{ position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(8,12,26,0.97), rgba(8,12,26,0.88))" }} />
        <div style={{ position: "relative", zIndex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 28 }}>
            <div style={{ width: 64, height: 64, background: `linear-gradient(135deg, ${T.orange}, ${T.gold})`, borderRadius: 20, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit', sans-serif", fontWeight: 900, fontSize: 24, color: "#fff" }}>Ti</div>
            <div>
              <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 28, fontWeight: 900, color: "#fff" }}><span style={{ color: T.orange }}>Talent</span> Ignition</div>
              <div style={{ fontSize: 10, color: "#4A5568", letterSpacing: 2.5, textTransform: "uppercase" }}>Ignite · Evolve · Excel</div>
            </div>
          </div>
      ...
      <Section bg={T.cream} py="80px 40px">
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 48 }}>
          <div>
            ...
            <p style={{ fontSize: 14.5, color: T.muted, marginBottom: 32, lineHeight: 1.75 }}>Fill in the form and a member of our team will get back to you within 24 hours — or you can call/WhatsApp us directly.</p>

            {sent ? (
              <div style={{ background: `${T.green}12`, border: `2px solid ${T.green}30`, borderRadius: 20, padding: "32px 28px", textAlign: "center" }}>
                <div style={{ fontSize: 48, marginBottom: 14 }}>✅</div>
                <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 22, fontWeight: 800, color: T.green, marginBottom: 8 }}>Message Sent!</div>
          ...
          <div>
            ...
            <p style={{ fontSize: 14.5, color: T.muted, marginBottom: 32, lineHeight: 1.75 }}>Prefer to talk? Call us, WhatsApp us, or visit our office in Srikakulam, Andhra Pradesh.</p>
            ...
            <div style={{ background: T.ink, borderRadius: 20, padding: "24px 22px", marginTop: 24 }}>
              <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: 16, fontWeight: 800, color: "#fff", marginBottom: 6 }}>School Partnership Enquiries</div>
              <p style={{ fontSize: 13.5, color: "#4A5568", lineHeight: 1.75, marginBottom: 16 }}>Want to bring Talent Ignition to your school? We offer institutional packages with teacher training and bulk enrollment.</p>
              <Btn size="sm" variant="outline" style={{ borderColor: T.borderDark, color: "#94A3B8" }}>enquiry@talentignition.in</Btn>
            </div>
          </div>
        </div>
      </Section>
          ...
          {RDCA.map(r => (
            <div key={r.id} style={{ background: r.bg, border: `2px solid ${r.border}`, borderRadius: 20, padding: "22px 18px", cursor: "pointer", transition: "all 0.25s" }}
              ...
              <div style={{ marginTop: 14, height: 4, background: `${r.color}20`, borderRadius: 2 }}>
                <div style={{ height: "100%", background: r.color, borderRadius: 2, width: `${Math.floor(Math.random() * 40 + 50)}%`, transition: "width 1s ease" }} />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Welcome Back 👋" subtitle="Log in to access your RDCA dashboard.">
      ...
      <button style={{ width: "100%", padding: "13px 16px", borderRadius: 12, border: `2px solid ${T.border}`, background: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'Outfit', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, color: T.ink, transition: "all 0.2s" }}
        onMouseEnter={e => { e.currentTarget.style.background = T.cream; e.currentTarget.style.borderColor = T.orange + "40"; }}
...
export default function TalentIgnition() {
  ...
  return (
    <div style={{ fontFamily: "'Outfit', 'DM Sans', sans-serif", background: T.cream, color: T.ink, minHeight: "100vh" }}>
      <style>{`
